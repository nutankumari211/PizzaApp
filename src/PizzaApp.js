...
const PizzaApp = ({ orders, totalDelivered }) => {
  ...
  const canHandleMoreOrders =
    orders.filter((order) => order.stage >= 0 && order.stage <= 2).length < 10;
  ...
  return (
    <div>
      {canHandleMoreOrders ? (
        ...
        <h2>Not taking any order for now. Maximum orders reached.</h2>
      )}

      <div>
        <h2 className="pizza-section">Pizza Stages Section</h2>
            ...
            <div key={stage} className={`stage-col stage-${stage}`}>
              ...
              {ordersByStages[stage].map((order) => {
                ...
                return (
                  <div
                    key={order.id}
                    className={`pizzas-card ${
                      order.stage === 3 ? 'picked' : ''
                    } ${
                      order.stage !== 3 && timeSpent > '3 min'
                        ? 'red-card'
                        : ''
                    }`}
                  >
      ...
      <div>
        <h2 className="main-section">Main Section</h2>
        <table>
          <thead>
            <tr>
              <th>Order Id</th>
...
export default connect(mapStateToProps)(PizzaApp);
